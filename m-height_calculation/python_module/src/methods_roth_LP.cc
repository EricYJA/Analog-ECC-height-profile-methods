...
#ifdef HAVE_HIGHS
double h_m_roth_primal_lp_highs(const Eigen::MatrixXd& G, int m, double early_quit_threshold) {
    return roth_lp(G, m, early_quit_threshold, Backend::Highs, true);
}
double h_m_roth_dual_lp_highs(const Eigen::MatrixXd& G, int m, double early_quit_threshold) {
    return roth_lp(G, m, early_quit_threshold, Backend::Highs, false);
}
#endif
