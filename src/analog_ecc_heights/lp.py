...
def h_m_jiang_simplified_lp(
    ...
    """Compute the simplified Jiang-style LP, returning min(height, threshold).
...
def h_m_roth_primal_lp(
    ...
    Requires 0 <= m < n. At m=0, returns min(h_0, threshold),
    where h_0 is zero for an all-zero G and one otherwise.
    Backends: python (default), cpp-glpk, cpp-highs.
    """
